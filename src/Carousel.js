...
define([
    ...
    Brickrouge.Carousel = new Class({
        ...
        initialize: function (el, options) {
            ...
            this.dots = el.querySelectorAll('.carousel-dots .dot')
            ...
            if (this.options.positionPattern && !this.positionEl) {
                ...
                this.element.appendChild(this.positionEl)
            }
        ...
        setDots: function (number) {
            var dots = new Element('div.carousel-dots')
                , replaces = this.element.querySelector('.carousel-dots')
                , dotStyle = this.options.dotStyle

            for (var i = 0; i < number; i++) {
                dots.adopt(new Element('div.dot', {

                    html: dotStyle == 'numeric' ? i + 1 : '&bull;',
                    'data-position': i
        ...
        setMethod: function (method) {
            ...
            if (method.next) {
                this.next = method.next
                if (method.prev) {
                    this.prev = method.prev
        ...
        updatePositionDisplay: function () {
            ...
            if (dotsEl) {
                ...
                dotsEl[position].classList.add('active')
            }
        }
    })
    ...
    Brickrouge.Carousel.Methods = {
        ...
        slide: {
            ...
            initialize: function () {
                var view = this.view = new Element('div', {
                    styles: {
                        position: 'absolute',
                        left: 0,
                        top: 0
    ...
    Brickrouge.register('Carousel', function (element, options) {

        return new Brickrouge.Carousel(element, options)
