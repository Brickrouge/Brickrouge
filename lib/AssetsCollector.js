define([
	...
	function filterMissing(assets)
	{
		...
		assets.css.forEach(url => {
			...
			if (availableCSS.indexOf(url) !== -1) return
		...
		assets.js.forEach(url => {
			...
			if (availableJS.indexOf(url) !== -1) return
	...
	{
		let filtered = filterMissing(assets)
		let promises = []
		...
		Promise.all(promises).then(resolved).catch(rejected || function(rejected) {
