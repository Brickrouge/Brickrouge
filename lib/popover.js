...
Brickrouge.Popover = new Class({
	...
	changePlacement: function(placement)
	{
		...
		el.addClass(placementClasses[placement])
	},
	...
	computeBestPlacement: function(anchorBox, w, h)
	{
		...
		switch (placement)
		{
			case 'right': if (fitAfter()) return placement; break
			case 'left': if (fitBefore()) return placement; break
			case 'top': if (fitAbove()) return placement; break
			case 'bottom': return placement
		}
...
Brickrouge.Popover.from = function(options)
{
	...
	if (actions == 'boolean')
	{
		actions = [ new Element('button.btn.btn-secondary.btn-cancel[data-action="cancel"]', { html: Locale.get('Popover.cancel') || 'Cancel' })
