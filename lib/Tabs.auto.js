...
define([
	 ...
	 */
	document.body.addDelegatedEventListener('[data-toggle="tab"]:not(.disabled)', 'click', function (ev, el) {
		...
		active = el.closest('.nav-tabs').getFirst('.active')
		...
		active = pane.closest('.tab-content').getFirst('.active')
	...
	Brickrouge.observe('update', function (fragment) {
		...
		fragment.getElements('.nav-tabs-fill').each(function (nav) {
