...
Brickrouge.Form = new Class({
			...
			Object.each(original, function(message, id) {
				...
				if (typeOf(id) == 'string' && id != '_base')
				{
					...
					if (typeOf(el) == 'collection')
					{
						...
						field = parent.getParent('.form-group')
						...
						else
						{
							...
							{
								document.id(el[i]).addClass('has-danger')
							}
						}
					}
					else if (el)
	...
	{
		var el = this.element
		, alerts = el.getElements('div.alert.dismissible')
console.log('alerts:', alerts)
		if (alerts)
		{
			alerts.destroy()
		}
console.log('has:', el.getElements('.has-danger'))
		el.getElements('.has-danger').removeClass('has-danger')
	},
